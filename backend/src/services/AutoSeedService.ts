...
export class AutoSeedService {
      ...
      const providers = [
        ...
        },
        {
          ...
          configuration: {
            ...
            dataQuality: 'high',
            supportedOperations: [
              'enrich-company', 'search-companies'
            ]
          },
        },
      ];
  ...
  private static async seedProviderFeatures(): Promise<void> {
    ...
    const surfe = await prisma.provider.findUnique({ where: { name: 'surfe' } });
    const apollo = await prisma.provider.findUnique({ where: { name: 'apollo' } });
    const hunter = await prisma.provider.findUnique({ where: { name: 'hunter' } });
    const betterenrich = await prisma.provider.findUnique({ where: { name: 'betterenrich' } });
    const companyenrich = await prisma.provider.findUnique({ where: { name: 'companyenrich' } });
  ...
  private static async seedBetterEnrichFeatures(providerId: number): Promise<void> {
    const features = [
  ...
  private static async seedCompanyEnrichFeatures(providerId: number): Promise<void> {
    const features = [
      {
